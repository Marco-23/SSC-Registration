...
Depending on the chosen options, the pipeline may generate:

- `ssc_fragments/`  
  Accepted SSC fragments
- `rejected_fragments/`  
  Final candidate fragments that failed SSC
- `initial_frames/`  
  Initial GPS-time frames
- `ssc_summary.csv`  
  Fragment-level summary
- `ssc_fragments_preview.ply`  
  Colored preview of the saved fragments

---

## How SSC works in this implementation

The implemented SSC workflow follows this sequence:

1. Load the MLS point cloud
2. Detect the GPS time field
3. Sort all points by GPS time
4. Split the point cloud into initial time-based frames
5. Build a candidate fragment from one or more consecutive initial frames
6. Estimate normals on the whole current candidate fragment
7. Orient normals consistently
8. Project normal directions onto the SSC semi-sphere
9. Cluster them using the five canonical seed directions
10. Evaluate fragment quality using seed population and displacement metrics
11. Accept the fragment or append the next initial frame and test again

The five canonical seed directions are:
...
These are used to evaluate whether the candidate fragment contains sufficiently distributed orthogonal geometric structure.

---

## Article-aligned behavior vs. project-specific behavior

This repository is based on the method described in the paper, but the practical implementation may include additional engineering rules for workflow use.

### Article-aligned parts

- GPS-time-based initial splitting
- adaptive fragment growth
- whole-fragment normal recomputation
- semi-sphere projection of normal directions
- five-seed clustering
- quality evaluation based on seed distribution

### Project-specific parts

- configurable command-line thresholds
- optional debug exports
- performance-oriented implementation changes
- optional automatic acceptance above a user-defined maximum fragment extent, such as `205 m`

This means the repository should be understood as an **article-aligned practical implementation**, not necessarily as a strict one-to-one reproduction of the exact published setup.
